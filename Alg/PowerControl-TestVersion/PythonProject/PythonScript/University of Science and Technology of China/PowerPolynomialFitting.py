...
使用方法:
  python PowerPolynomialFitting.py <data.csv>
"""
...
def load_data(csv_path):
    ...
    I = pd.to_numeric(df.iloc[:, 1], errors='coerce').values     # 第二列: I (电流, A)
    w = pd.to_numeric(df.iloc[:, 2], errors='coerce').values     # 第三列: W (转速, rad/s)
...
def fit_model(csv_path):
    ...
    save_path = os.path.join(save_dir, 'fitting_result.png')
